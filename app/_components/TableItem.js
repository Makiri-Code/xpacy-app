...
export default function TableItem({ service_type, address, scheduled_date, service_status, }) {
    ...
    return (
        <tr className=" border-b border-gray-300">
            ...
            <td className="px-4">
                <span className={`${statusBg[service_status.toLowerCase()] || ""} px-1.5 py-1 w-max flex items-center justify-center rounded-full  text-sm font-bold`}>{service_status}</span>
            </td>
        </tr>
    )
}
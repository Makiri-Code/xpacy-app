                        ...
                        {bookings.map((booking) => (
                            <tr key={booking.id || booking._id} className="hover:bg-gray-50 transition-colors">
                                <td className="p-4 font-medium text-gray-900">
                                    {booking.property?.property_name || "Unknown Property"}
                                </td>
                                <td className="p-4">
                                    {booking.user?.firstname} {booking.user?.lastname}
                                </td>
                                <td className="p-4 text-gray-500">
                                    {booking.createdAt || booking.start_date ? format(new Date(booking.createdAt || booking.start_date), "MMM dd, yyyy") : "-"}
                                </td>
                                <td className="p-4">
                                     <span className={`px-2.5 py-1 rounded-full text-xs font-bold capitalize ${
                                        booking.payment_status?.toLowerCase() === 'paid' || booking.payment_status?.toLowerCase() === 'completed' 
                                ...
                                <td className="p-4 text-right font-bold text-gray-900">
                                    {formatCurrency(booking.amount || booking.property?.property_price)}
                                </td>
                            </tr>
                        ))}
                    </tbody>
